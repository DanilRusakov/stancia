		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if ($this->config->get('config_order_mail')) {
					...
					$data['link'] = '';
					$data['download'] = '';
					$order_comment = $this->db->query("SELECT payment_field24 FROM " . DB_PREFIX . "order_simple_fields WHERE order_id = '" . (int)$order_id . "'");
					if($order_comment!=0){
						$data['simpl_comment'] = $order_comment->row['payment_field24']; 
					}
