		...
		<?php if ($product['thumb']) { ?>
			...
			<div class="image <?php if($theme_options->get( 'product_image_effect' ) == '1') { echo 'image-swap-effect'; } ?>">
				<?php if (isset($product['quantity'])) { ?>
					<? if ($product['stock_status_id']=='8' && $product['quantity'] <= 0){ ?>
						<span style="color: red;     position: absolute;    right: 0;    z-index: 1;    top: 15%;"><strong>Предзаказ</strong></span>
					<? }else{ ?>
						<?php if ($product['quantity'] <= 0) { ?>
						<span style="color: red;     position: absolute;    right: 0;    z-index: 1;    top: 15%;     font-size: 16px;"><strong>Нет в наличии</strong></span>
						<?php }} ?>
				<?php } ?>
				...
				<a href="<?php echo $product['href']; ?>">
					<?php if($theme_options->get( 'product_image_effect' ) == '1') {
						...
						if($image_swap != '') echo '<img src="' . $image_swap . '" alt="'.($product['alt_text']!='') ? $product['alt_text'] : $product['name'].'" class="swap-image" />';
					} ?> 
					<?php if($theme_options->get( 'lazy_loading_images' ) != '0') { ?>
					<img src="image/catalog/blank.gif" data-echo="<?php echo $product['thumb']; ?>" alt="<?php echo ($product['alt_text']!='') ? $product['alt_text'] : $product['name']; ?>" title="<?php echo ($product['title_text']!='') ? $product['title_text'] : $product['name']; ?>" class="<?php if($theme_options->get( 'product_image_effect' ) == '2') { echo 'zoom-image-effect'; } ?>" />
                    <?php } else { ?>
					<img src="<?php echo $product['thumb']; ?>" alt="<?php echo ($product['alt_text']!='') ? $product['alt_text'] : $product['name']; ?>" title="<?php echo ($product['title_text']!='') ? $product['title_text'] : $product['name']; ?>" class="<?php if($theme_options->get( 'product_image_effect' ) == '2') { echo 'zoom-image-effect'; } ?>" />
                    <?php } ?>
				</a>
			</div>
		<?php } else { ?>
